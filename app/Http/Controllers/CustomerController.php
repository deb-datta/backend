...
{   
    ...
    {   
        $allexpence = Customer::select('id','name','cutomermobile','cutomeremail','companyname','username','password')->orderBy('id', 'desc')->paginate(15);
    ...
    {   
        if($request->user()->group_id==5){
            $clientids=$request->user()->companyid;
            
            $compusername=$request->user()->name;
            $allexpence = Company::where('id',$clientids)->get(['id','name']);
            ...
            $allexpence = Company::get(['id','name']);
        }
        if(count($allexpence) > 0){
            return response()->json($allexpence, 200);
        }
    ...
    {   
        if($request->user()->group_id==5){
            ...
            $compusername=$request->user()->name;
            $allexpence = Customer::where('companyid',$clientids)->get(['id','name']);
        }else if($request->user()->group_id==4){
            $clientids=$request->user()->companyid;
            ...
            $allexpence = Customer::get(['id','name']);
        }
        if(count($allexpence) > 0){
            return response()->json($allexpence, 200);
        }
    ...
    {   
        $allexpence = Company::select('id','name','address','email')->orderBy('id', 'desc')->paginate(15);
        if(count($allexpence) > 0){
            return response()->json($allexpence, 200);
        }
        return response()->json([
    ...
    public function addCustomercompany(Request $request)
    {
        $this->validate($request, [            
            ...
            $accout->countrycode = $request->countrycode;
    ...
    public function editCustomercompany(Request $request)
    {
        ...
        $id=$request->id;
            $accout = Company::where("id",$id)->get()->first();
            $accout->name = $request->name;
            $accout->email = $request->email;
            $accout->address = $request->address;           
            $accout->countrycode = $request->countrycode;
    ...
    public function gettheCompany($id)
    {
        ...
        $alldatas = Company::where("id",$id)->get()->first();
        if(count($alldatas) > 0){
    ...
    public function addCustomer(Request $request)
    {
        ...
        $getdatassdsd=Company::where("id",$compida)->get()->first();
        if($getdatassdsd){
            $companyid=$request->companyid;
            $accout = new Customer;

            $accout->name = $request->name;
            $accout->cutomermobile = $request->mobile;
            $accout->cutomeremail = $request->email;
            $accout->companyid = $request->companyid;
            $accout->companyname= $getdatassdsd->name;
            ...
            if($datasaved){
                $getidnooa =Customer::where('name',$request->name)->where('cutomermobile',$request->mobile)->first();
    ...
    public function editfnlCustomer(Request $request)
    {
        ...
        if($getdatassdsd){
            $id=$request->id;
            $companyid=$request->companyid;
            $accout = Customer::where("id",$id)->get()->first();
            //$accout->name = $request->name;
            $accout->cutomermobile = $request->mobile;
            $accout->cutomeremail = $request->email;
            $accout->companyid = $request->companyid;
    ...
    public function gettheCustomer($id)
    {
        ...
        $alldatas = Customer::where("id",$id)->get()->first();
        if(count($alldatas) > 0){
            return response()->json($alldatas, 200);
        }
        return response()->json([
    ...
    public function inchargeCompany($id){
        $alldatas = Customer::where('companyid',$id)->get(['id','name']);
        if(count($alldatas) > 0){
            return response()->json($alldatas, 200);
        }
        return response()->json([
